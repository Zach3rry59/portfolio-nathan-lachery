...
export function ProfileScene({ profile, profileKey }) {
  return <Scene id="profile" title="01 — LE PROFIL" caption={profile.caption} className="profile-section"><div className="profile-intro"><h2 id="profile-title">{profile.title[0]}<br/><span className="muted">{profile.title[1]}</span></h2><div className="intro-columns"><div className="profile-identity"><span className="profile-symbol" aria-hidden="true">{profile.icon}</span><h3>{profile.role}</h3><span className="mono">DOUAI (59) · PERMIS B</span></div><div className="profile-copy"><p>{profile.introduction}</p><p className="objective"><span className="status-dot"/>{profile.objective}</p><a className="text-link" href={`/cv/${profileKey}.pdf`} target="_blank" rel="noopener noreferrer">Consulter le CV {profile.label} ↗</a></div></div></div></Scene>;
}
...
function Timeline({ entries, training }) { return entries.map(entry => <article className="timeline-entry" key={`${entry.date}-${entry.title}`}><div className="entry-date">{entry.date}</div><div><h3>{entry.title}</h3><p className="entry-organization">{entry.organization}</p><p className="entry-description">{training ? entry.detail : entry.description}</p></div></article>); }
export function JourneyScene({ profile, training = false }) {
  ...
  return <Scene id={id} title={training ? '04 — LES FORMATIONS' : '03 — LES EXPÉRIENCES'} caption="APPRENDRE · PRATIQUER · ÉVOLUER">
    <div className="journey">
      ...
      <div className="journey-content"><div className="timeline-group"><h3 className="timeline-title">{training ? 'Formations' : 'Expériences'} <span>{training ? '02' : '01'}</span></h3><Timeline entries={training ? profile.training : profile.experiences} training={training}/></div></div>
    </div>
  </Scene>;
}
